...
func (s Float64s) Coalesce(other any) Series {
	...
	case NAs:
		...
		case s.Len() == 1 && o.Len() == 1:
			...
			result[0] = s.Data_[0]
			return Float64s{IsNullable_: s.IsNullable_, NullMask_: resultNullMask, Data_: result, Ctx_: s.Ctx_}
		case s.Len() == 1:
			resultSize := o.Len()
			result := make([]float64, resultSize)
			resultNullMask := naOperandNullMask(s.IsNullable_, s.NullMask_, true, resultSize)
