...
func seriesGroupBy(
	...
	// Initialize the maps
	maps := make([]map[int64][]int, enchanter.THREADS_NUMBER)
	for i := 0; i < enchanter.THREADS_NUMBER; i++ {
		maps[i] = make(map[int64][]int, enchanter.DEFAULT_HASH_MAP_INITIAL_CAPACITY)
	}

	var nulls [][]int
	if hasNulls {
		nulls = make([][]int, enchanter.THREADS_NUMBER)
		for i := 0; i < enchanter.THREADS_NUMBER; i++ {
			nulls[i] = make([]int, 0)
		}
	}
...
func seriesNullKey(map_ map[int64][]int, seed int64) int64 {
	for {
		...
		seed = enchanter.HASH_MAGIC_NUMBER + (seed << 13) + (seed >> 4)
	}
	return seed
}
