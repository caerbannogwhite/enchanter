...
func (s Strings) Context() *enchanter.Context {
	return s.ctx
}
...
func (s Strings) SortOrder() enchanter.SeriesSortOrder {
	return s.sorted
}

// Err returns the error carried by the series: always nil, a
// Strings series is never in an error state.
func (s Strings) Err() error {
	return nil
}

// Return the partition of the series.
func (s Strings) Partition() SeriesPartition {
	return s.partition
}
...
func (s Strings) PackedNullMask() []uint8 {
	return s.nullMask
}

func (s Strings) NullMask() []bool {
...
func (s Strings) Append(v any) Series {
	...
	switch v := v.(type) {
	case nil:
		s.data = append(s.data, s.ctx.StringPool.Put(enchanter.NA_TEXT))
...
func (s Strings) Slice(start, end int) Series {
	if start < 0 || end < start || end > s.Len() {
		return Errors{fmt.Sprintf("Strings.Slice: invalid interval [%d, %d) for a series of length %d", start, end, s.Len())}
	}
...
// series.
func (s Strings) TakeIndices(indices []int) Series {
	for _, v := range indices {
		if v < 0 || v >= s.Len() {
			return Errors{fmt.Sprintf("Strings.TakeIndices: index %d is out of range", v)}
		}
	}
	return s.filterIntSlice(indices, false)
}
...
func (s Strings) Map(f enchanter.MapFunc) Series {
	if len(s.data) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Strings) MapNull(f enchanter.MapFuncNull) Series {
	if len(s.data) == 0 {
		return s
	}
