...
func (s Int64s) Context() *enchanter.Context {
	return s.ctx
}
...
func (s Int64s) SortOrder() enchanter.SeriesSortOrder {
	return s.sorted
}

// Err returns the error carried by the series: always nil, a
// Int64s series is never in an error state.
func (s Int64s) Err() error {
	return nil
}

// Return the partition of the series.
func (s Int64s) Partition() SeriesPartition {
	return s.partition
}
...
func (s Int64s) PackedNullMask() []uint8 {
	return s.nullMask
}

func (s Int64s) NullMask() []bool {
...
func (s Int64s) Slice(start, end int) Series {
	if start < 0 || end < start || end > s.Len() {
		return Errors{fmt.Sprintf("Int64s.Slice: invalid interval [%d, %d) for a series of length %d", start, end, s.Len())}
	}
...
// series.
func (s Int64s) TakeIndices(indices []int) Series {
	for _, v := range indices {
		if v < 0 || v >= s.Len() {
			return Errors{fmt.Sprintf("Int64s.TakeIndices: index %d is out of range", v)}
		}
	}
	return s.filterIntSlice(indices, false)
}
...
func (s Int64s) Map(f enchanter.MapFunc) Series {
	if len(s.data) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Int64s) MapNull(f enchanter.MapFuncNull) Series {
	if len(s.data) == 0 {
		return s
	}
