...
func (s Int64s) ArrowArray() arrow.Array {
	return buildArrowInt64(s.ctx.Allocator, s.data, s.isNullable, s.nullMask)
}
...
func (s Int64s) DataAsNullable() any {
	data := make([]enchanter.NullableInt64, len(s.data))
	for i, v := range s.data {
		data[i] = enchanter.NullableInt64{Valid: !s.IsNull(i), Value: v}
	}
	return data
}
	...
	worker := func(threadNum, start, end int, map_ map[int64][]int) {
		...
		for _, h := range keys[start:end] { // keys is defined outside the function
			for _, index := range otherIndeces[h] { // otherIndeces is defined outside the function
				newHash = s.data[index] + enchanter.HASH_MAGIC_NUMBER + (h << 13) + (h >> 4)
