...
// Bools series is never in an error state.
func (s Bools) Err() error {
	return nil
}
...
func (s Bools) PackedNullMask() []uint8 {
	return s.nullMask
}
...
func (s Bools) Slice(start, end int) Series {
	if start < 0 || end < start || end > s.Len() {
		return Errors{fmt.Sprintf("Bools.Slice: invalid interval [%d, %d) for a series of length %d", start, end, s.Len())}
	}
...
// series.
func (s Bools) TakeIndices(indices []int) Series {
	for _, v := range indices {
		if v < 0 || v >= s.Len() {
			return Errors{fmt.Sprintf("Bools.TakeIndices: index %d is out of range", v)}
		}
	}
	return s.filterIntSlice(indices, false)
}
...
func (s Bools) Map(f enchanter.MapFunc) Series {
	if len(s.data) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Bools) MapNull(f enchanter.MapFuncNull) Series {
	if len(s.data) == 0 {
		return s
	}
