...
func (s Times) Context() *enchanter.Context {
	return s.ctx
}
...
func (s Times) SortOrder() enchanter.SeriesSortOrder {
	return s.sorted
}

// Err returns the error carried by the series: always nil, a
// Times series is never in an error state.
func (s Times) Err() error {
	return nil
}

// Return the partition of the series.
func (s Times) Partition() SeriesPartition {
	return s.partition
}
...
func (s Times) PackedNullMask() []uint8 {
	return s.nullMask
}

func (s Times) NullMask() []bool {
...
func (s Times) Slice(start, end int) Series {
	if start < 0 || end < start || end > s.Len() {
		return Errors{fmt.Sprintf("Times.Slice: invalid interval [%d, %d) for a series of length %d", start, end, s.Len())}
	}
...
// series.
func (s Times) TakeIndices(indices []int) Series {
	for _, v := range indices {
		if v < 0 || v >= s.Len() {
			return Errors{fmt.Sprintf("Times.TakeIndices: index %d is out of range", v)}
		}
	}
	return s.FilterIntSlice(indices, false)
}
...
func (s Times) Map(f enchanter.MapFunc) Series {
	if len(s.data) == 0 {
		return s
	}
...
// Apply the given function to each element of the series.
func (s Times) MapNull(f enchanter.MapFuncNull) Series {
	if len(s.data) == 0 {
		return s
	}
